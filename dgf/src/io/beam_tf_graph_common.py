...
def tf_example_to_edge(
    ...
    schema: Optional[schema_lib.EdgeSchema] = None,
    ignore_keys: tuple[str, ...] = (
  ...
  return distributed_graph.Edge(
      source=edge_source, target=edge_target, id=edge_id, features=edge_features
  )
...
def edge_to_tf_example(
  ...
  if edge.id is not None and edge_id_column is not None:
