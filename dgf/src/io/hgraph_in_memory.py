...
def tfgnn_schema_to_schema(
  ...
  analyse_schema_lib.fix_schema(
      schema,
      create_pound_id_as_fall_back=True,
      fix_shape=fix_shape,
      tf_example=True,
  )
  return schema
...
def _create_empty_feature_array(
    num_rows: int, shape: schema_lib.Shape, format: schema_lib.FeatureFormat
) -> np.ndarray:
  tf_dtype = feature_format_lib.FEATURE_FORMAT_TO_TF_DTYPE[format]
  dtype = tf_dtype.as_numpy_dtype
  if shape is None:
    shape = ()
  resolved_shape = [d if (d is not None and d != -1) else 1 for d in shape]
  full_shape = (num_rows, *resolved_shape)
  if format == schema_lib.FeatureFormat.BYTES:
    fill_value = b""
    return np.full(full_shape, fill_value, dtype=dtype)
  else:
...
def read_graphai_hgraph(
  ...
  Args:
    ...
    remove_dangling_edges: If False (default), fails if an edge is dangling. If
      True, silently filters out any dangling edges.
  ...
  def _feature_format_to_container_dtype(
    ...
    if container_type == HGraphContainerType.TF_RECORD:
      ...
      raise ValueError(
          ...
          " HGraphContainerType.TF_RECORD, "
          "and HGraphContainerType.AVRO are supported."
      )
  ...
  for nodeset_name, nodeset_def in schema.node_sets.items():
    ...
    node_features, num_nodes = _read_container(
        ...
        columns=columns,
        verbose=verbose,
        key_column=node_id_column,
    )

    # Index the nodeset ids
    if verbose:
      log.info("Building index for %s", nodeset_name)
  ...
  for edge_set_name, edge_set in schema.edge_sets.items():
    if verbose:
      log.info("Reading edgeset %s from %s", edge_set_name, path)
    paths = shard_lib.list_paths(
    ...
    if research_node_format:
      raise ValueError("research_node_format not supported")
    else:
      ...
      source_ids = raw_edges[KEY_SOURCE]
      target_ids = raw_edges[KEY_TARGET]
      features = raw_edges

    if verbose:
      log.info("Computing adjacency for %s", edge_set_name)

    source_mapper = nodeset_mapping[edge_set.source]
    target_mapper = nodeset_mapping[edge_set.target]
    ...
    if remove_dangling_edges:
      ...
      if missmatch_src != -1:
        ...
        raise ValueError(
            f"Node ID {bad_id!r} not found in nodeset {edge_set.source!r}."
        )
      if missmatch_trg != -1:
        bad_id = target_ids[missmatch_trg].item().decode()
        raise ValueError(
            f"Node ID {bad_id!r} not found in nodeset {edge_set.target!r}."
        )

    if research_node_format:
      for feature_name, feature_def in edge_set.features.items():
        # TODO(gbm): Add reader for edge features with research node format.
        log.warning(
            "Edge features are not supported with research_node_format. "
            "Feature %r will be empty.",
            feature_name,
        )
        features[feature_name] = _create_empty_feature_array(
            num_rows=adjacency.shape[1],
            shape=feature_def.shape,
            format=feature_def.format,
        )
    else:
      # Deleting source and target from features because they have been
      # converted
      # to adjacencies.
