...
class EarlyStoppingMonitorTest(absltest.TestCase):
  ...
  def test_improving_loss(self):
    ...
    monitor.add_loss(1.0)
    self.assertFalse(monitor.should_stop())

    monitor.add_loss(0.9)
    self.assertFalse(monitor.should_stop())

    monitor.add_loss(0.8)
    self.assertFalse(monitor.should_stop())
