...
class GNNDatasetPreparator:
  ...
  Attributes:
    ...
    skip_overflow_padding_error: If padding is set, the merging stage can fail
      if the "padding" is not large enough. If
      skip_overflow_padding_error=True, such batches are skipped. If
      skip_overflow_padding_error=False, an error is raised.
  ...
  def __post_init__(self):
    ...
    self.nodeset_timestamp_features = (
        temporal_util.nodeset_timestamp_features(self.schema)
    )
    self.edgeset_timestamp_features = (
        temporal_util.edgeset_timestamp_features(self.schema)
    )


  def _get_transform_configs(
      self,
  ) -> Tuple[
      Optional[timeseries_transform.PadAndCapTimeseriesConfig],
      Optional[timeseries_transform.TimestampFeatureExtractorConfig],
  ]:
    pad_and_cap_config = (
    ...
    return pad_and_cap_config, timestamp_config
  ...
  def prepare_from_existing_one(self, other: "GNNDatasetPreparator"):
    ...
    pad_and_cap_config, timestamp_config = self._get_transform_configs()
